...
```bash
lambda-java17-1$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-java17-1$ sam local start-api
lambda-java17-1$ curl http://localhost:3000/
```
...
```bash
lambda-java17-1$ sam logs -n HelloWorldFunction --stack-name lambda-java17-1 --tail
```
...
```bash
lambda-java17-1$ cd HelloWorldFunction
...
```bash
sam delete --stack-name lambda-java17-1
```
